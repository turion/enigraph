...
class DirNode(enigtree.Node):
	...
	def __init__(self, path, *args, **kwargs):
		...
		self._isfile = None
		self.path = path
		enigtree.Node.__init__(self, data=self.init_data(*args, **kwargs))
	...
	def childs(self):
		...
		if self._childs_done:
			return self._childs
		else:
			self_type = type(self)
			if self.isdir:
				self._childs = [self_type(os.path.join(self.path, child)) for child in os.listdir(self.path) if self.child_accepted(child)] # Hopefully ensures that the children will be instances of the same class, if inherited
			else:
				self._childs = []
			self.sort_childs()
